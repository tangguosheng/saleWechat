			...
			<table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
				<thead>
					<tr>
						...
						<th class="head0">
							<a href="<?php if($_GET['order'] == 'sub_time' && $_GET['type'] == 'desc'){ echo U('index?order=sub_time&type=asc'); }else{ echo U('index?order=sub_time&type=desc'); }?>">
							关注日期
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php if($vo['sub_time'] > 0): echo (date("Y-m-d",$vo["sub_time"])); else: ?>-<?php endif; ?></td>
