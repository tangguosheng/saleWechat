...
<body style=" background:#eee">
	...
	<div class="index-cate">
		<ul>
			<?php if(is_array($cates)): $i = 0; $__LIST__ = $cates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['pid'] == 0): ?><li>
					<a href="<?php echo U('all?cate_id='.$vo['id']);?>"><img src="<?php echo ($vo["pic"]); ?>" /><?php echo ($vo["name"]); ?></a>
	...
	<div class="index-main">
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="index-item2" style="">
			
			<a href="<?php echo U('Index/product?id='.$vo['id']);?>">
				<img src="<?php echo ($vo["pic"]); ?>" style=" width:100%; float:none; height:auto;" />
			</a>
			<div class="title">
			<a href="<?php echo U('Index/product?id='.$vo['id']);?>" >	<?php echo ($vo["title"]); ?></a>
			</div>
			<div style="padding:5px; text-align:left; line-height:30px; background:#fff;">
				<!--库存:<?php echo ($vo["stock"]); ?>-->
				<span style=" font-size:20px; color:red;">￥<?php echo ($vo["price"]); ?></span>
				<!--div class="price" style=" line-height:20px; float:right;">
				赠<?php echo ($_CFG["site"]["points_name"]); ?>:<?php echo ($vo["points"]); ?>
				</div-->
				<span class="right" style=" float:right">
					<a href="<?php echo U('Index/product?id='.$vo['id']);?>" style=" padding:3px 5px; background:red; color:#fff;">立即购买</a>
				</span>
				<div style="clear:both"></div>
			</div>
